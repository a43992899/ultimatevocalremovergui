...
class MainWindow():
    def __init__(self, inputPaths, export_path):
        ...
        self.save_format_var = DEFAULT_DATA['save_format']
    ...
    def create_sample(self, audio_file, sample_path=SAMPLE_CLIP_PATH):
        ...
        clip_duration = int(self.model_sample_mode_duration_var)
    ...
    def process_start(self):
        ...
        # except Exception as e:
        #     self.error_log_var = error_text(self.chosen_process_method_var, e)
        #     print(f'\n\n{PROCESS_FAILED}')
        #     print(time_elapsed())
        #     playsound(FAIL_CHIME) if self.is_task_complete_var else None
        #     print(f'\n{self.error_log_var}')
...
class AudioTools():
    def __init__(self, audio_tool):
        ...
        self.is_normalization = root.is_normalization_var
        self.is_testing_audio = f"{time_stamp}_" if root.is_testing_audio_var else ''
        self.save_format = lambda save_path:save_format(save_path, root.save_format_var, root.mp3_bit_set_var)
        self.align_window = TIME_WINDOW_MAPPER[root.time_window_var]
        self.align_intro_val = INTRO_MAPPER[root.intro_analysis_var]
        self.db_analysis_val = VOLUME_MAPPER[root.db_analysis_var]
        self.is_save_align = root.is_save_align_var#
        self.is_match_silence = root.is_match_silence_var#
        self.is_spec_match = root.is_spec_match_var
        
        self.phase_option = root.phase_option_var#
        self.phase_shifts = PHASE_SHIFTS_OPT[root.phase_shifts_var]
    ...
    def pitch_or_time_shift(self, audio_file, audio_file_base):
        ...
        rate = float(root.time_stretch_rate_var) if self.audio_tool == TIME_STRETCH else float(root.pitch_rate_var)
        is_pitch = False if self.audio_tool == TIME_STRETCH else True
        if is_pitch:
            is_time_correction = True if root.is_time_correction_var else False
